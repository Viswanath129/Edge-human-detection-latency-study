...
def main():
    ...
    # Use absolute path to ensure it finds the file regardless of where script is called from
    ...
    # Change to the plots directory to save results
    os.chdir(script_dir)
    ...
    # 4. FPS vs Precision (if multiple precisions exist for yolov8n at 640x640)
    prec_df = df[(df['Resolution'] == '640x640') & (df['Model'] == 'yolov8n')]
    if len(prec_df) > 1:
        plt.figure(figsize=(8, 5))
        plt.bar(prec_df['Precision'], prec_df['Average_FPS'], color='orange')
        plt.title('FPS vs Precision (YOLOv8n, 640x640)')
        plt.ylabel('Average FPS')
        plt.savefig('fps_vs_precision.png')
        plt.close()
