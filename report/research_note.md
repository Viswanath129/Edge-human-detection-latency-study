...
|------------|-------|-----------|-----|------------------|-------------|
| 640x640 | YOLOv8n | FP32 | 9.07 | 110.25 | Lightweight edge model |
| 416x416 | YOLOv8n | FP32 | 20.14 | 49.64 | Faster Inference |
| 640x640 | YOLOv8s | FP32 | 3.86 | 259.30 | Balanced accuracy/speed |

## Key Observations

1. **Resolution Impact**: Decreasing the input size from 640x640 to 416x416 increases throughput by over 120% (from 9.07 FPS to 20.14 FPS) and decreases latency by ~55% (from 110.25 ms to 49.64 ms). This highlights input downscaling as one of the most effective knobs to trade off precision for latency budget on resource-constrained hardware.
2. **Model Complexity**: Elevating model capacity from YOLOv8 Nano (`YOLOv8n`) to Small (`YOLOv8s`) causes latency to increase by over 2.3x (from 110.25 ms to 259.30 ms) and cuts FPS by ~57.4% (from 9.07 FPS to 3.86 FPS). While `YOLOv8s` is expected to have higher detection robustness, its computational demand makes it challenging for real-time edge processing without acceleration.
...
## Conclusion
For real-time scenarios on standard edge CPUs, **YOLOv8n with 416x416 input resolution** provides the most viable operating point, yielding ~20 FPS and latency close to 50 ms. For stationary applications where high accuracy is crucial and lower frame rates are acceptable, **YOLOv8s with 640x640 input resolution** remains a viable candidate if paired with hardware-accelerated inference engines.
